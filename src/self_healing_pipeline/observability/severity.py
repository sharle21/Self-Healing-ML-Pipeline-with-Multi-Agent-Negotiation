...
class SeverityCalculator:
    ...
    def calculate(
        self, incident_type: IncidentType, telemetry: Telemetry, baseline: Telemetry | None = None
        ...
        if incident_type == IncidentType.DRIFT:
            return self._drift_severity(telemetry, baseline)
        elif incident_type == IncidentType.DATA_QUALITY:
            return self._data_quality_severity(telemetry, baseline)
        elif incident_type == IncidentType.COST_THRESHOLD:
            return self._cost_severity(telemetry, baseline)
        elif incident_type == IncidentType.LATENCY_BREACH:
            return self._latency_severity(telemetry, baseline)
        else:
            return 0.5, SeverityBreakdown(0.5, 0.5, 0.5, 0.5, 0.5)

    def _drift_severity(self, telemetry: Telemetry, baseline: Telemetry | None) -> tuple[float, SeverityBreakdown]:
        """Severity for DRIFT incidents.

        Components:
        - Impact: AUC drop (normalized to 0.20 = 1.0)
        ...
        impact = min(auc_drop / 0.20, 1.0)  # 20% drop = catastrophic
    ...
    def _data_quality_severity(self, telemetry: Telemetry, baseline: Telemetry | None) -> tuple[float, SeverityBreakdown]:
        """Severity for DATA_QUALITY incidents.

        Components:
        - Impact: Missing rate (normalized to 50% = 1.0)
        - Deviation: Schema violations (normalized to 100 = 1.0)
        - Urgency: Volume affected
        - Business risk: Data corruption impact
        """
        # Impact: Missing data
        impact = min(telemetry.data.missing_rate / 0.50, 1.0)  # 50% missing = catastrophic
    ...
    def _cost_severity(self, telemetry: Telemetry, baseline: Telemetry | None) -> tuple[float, SeverityBreakdown]:
    ...
    def _latency_severity(self, telemetry: Telemetry, baseline: Telemetry | None) -> tuple[float, SeverityBreakdown]:
        """Severity for LATENCY_BREACH incidents.

        Components:
        - Impact: P95 latency degradation
        - Deviation: P99 latency (tail latency)
        - Urgency: User experience impact
        - Business risk: SLA breach cost
        """
        # Impact: P95 increase
        baseline_p95 = baseline.system.latency_p95 if baseline else 85
        p95_increase = max(0, telemetry.system.latency_p95 - baseline_p95)
        impact = min(p95_increase / 200, 1.0)  # 200ms increase = catastrophic
        ...
        # Urgency: Assume SLA = 100ms P95
        sla_breach = 1.0 if telemetry.system.latency_p95 > 100 else 0.0
        urgency = sla_breach
